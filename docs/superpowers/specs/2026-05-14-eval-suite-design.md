...
### Activity idempotence and step-level reentrance

Temporal retries activities on transient failures. Two classes of problem to avoid:

1. **Append-style writes** (e.g., a separate `record_step` appending to `steps.jsonl`) duplicate rows under retry. Solved by removing all appenders: per-step metrics go into single-file canonical `step_<NNN>/metrics.json`; aggregated files (`steps.jsonl`, `summary.jsonl`) are derived in one-shot overwrites by `finalize_trial` and `finalize_sweep`.
2. **Mid-step crashes** between LLM call and action application can cause a retry to re-issue the (paid) model call and pick a different stochastic action, doubling spend and diverging from the abandoned attempt. Solved by step-level reentrance: detect a completed step on entry and short-circuit; for incomplete steps, write artifacts incrementally and use an atomic publish as the commit marker.
...
- Retry of a completed step is free and produces the exact recorded result.
- Retry of a step that crashed *before* the model call is fine — no money spent, fresh attempt.
- Retry of a step that crashed *during or after* the model call re-issues the model call and picks a different stochastic action. The cost is one duplicate inference per such crash. For the most common transient failure (Bedrock throttling, surfaced before any side effect), Bedrock SDK retries handle it inside the model call and no Temporal retry is needed.
...
@dataclass
class Action:
    kind: str                       # "press_buttons" | "navigate_to" | "wait" | ...
    args: dict                      # raw tool-call arguments
    buttons: list[str]              # actual GB buttons applied (post-expansion for navigate_to)
    frames_elapsed: int             # PyBoy frames advanced by this action
    success: bool | None = None     # for actions that can fail (e.g., navigate_to blocked)
    result_text: str | None = None  # short outcome description, e.g. "reached (5,3)" / "blocked by wall"
...
class StepMetrics:
    ...
    # Work counters: capture that "1 step" can mean very different amounts of effort
    decision_count: int             # number of emulator-affecting actions taken this step
...
class StepResult:
    ...
    # so they survive mid-step crashes. There is no manifest field on StepResult.
...
Harnesses write their per-step artifacts (prompt, response, screenshot, subagent traces, etc.) into `ctx.workdir` as soon as each is produced — `prompt.json` before the model call, `response.json` immediately after, `screenshot.png` and `memory_dump.json` after action execution. This is required for the step-level reentrance protocol described in §"Activity idempotence and step-level reentrance".
...
Work counters (because "one step" means different things across harnesses):
- `decision_count` — emulator-affecting actions taken this step
- `tool_call_count` — total tool invocations across all internal model calls
- `button_press_count` — raw GB buttons applied to the emulator (post-expansion of navigator paths, button-batched press_buttons calls, etc.)
...
Recording all the work counters in parallel lets us compare harnesses that bundle work very differently. A navigator harness may have low `steps_to_milestone` but high `button_press_count` per step; a single-button harness inverts that. Both are legitimate signals to weigh.
